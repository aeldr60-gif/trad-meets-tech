...
def plot_chord_quality_by_type(beat_csv: Path, outdir: Path, top_k: int = 8) -> None:
    ...
    has_col = pick_col(df, ["has_chord_here"])
    deg_cols = [c for c in df.columns if c.startswith("deg_")]

    if not (has_col and deg_cols):
        print("[WARN] beat_slots missing has_chord_here or degree columns; skipping melody degree plot.")
        return
    ...
    chord = d[d[has_col] == 1]
    nochord = d[d[has_col] == 0]

    if chord.empty or nochord.empty:
        print("[WARN] Not enough chord/no-chord rows for melody degree comparison; skipping.")
        return

    chord_sum = chord[deg_cols].sum()
    nochord_sum = nochord[deg_cols].sum()

    chord_prop = chord_sum / (chord_sum.sum() if chord_sum.sum() else 1.0)
    nochord_prop = nochord_sum / (nochord_sum.sum() if nochord_sum.sum() else 1.0)

    x = np.arange(len(deg_cols))
    width = 0.45

    fig = plt.figure(figsize=(14, 5))
    ax = fig.add_subplot(111)

    ax.bar(x - width / 2, chord_prop.values, width, label="Chord slots (has_chord_here=1)")
    ax.bar(x + width / 2, nochord_prop.values, width, label="No-chord slots (has_chord_here=0)")

    ax.set_xticks(x)
    ax.set_xticklabels(deg_cols, rotation=45, ha="right")
    ax.set_title("Melody Degree Distribution: Chord Slots vs No-Chord Slots (Proportions)")
    ax.set_ylabel("Proportion of melody notes (degree bins)")
    ax.legend()

    savefig(fig, outdir / "07_melody_degree_chord_vs_nochord.png")


# ----------------------------
# Plot 07: Heatmap of chord_nashville frequency by slot position (Top-N), split by tune type
# ----------------------------
def plot_chord_nashville_heatmap_by_type(beat_csv: Path, outdir: Path, tune_type: str, outname: str, top_n: int = 15) -> None:
    df = read_csv_safe(beat_csv)
    if df is None or df.empty:
        return
    ...
    if not (type_col and has_col and slot_col and spm_col and chord_col):
        print("[WARN] beat_slots missing required columns for chord_nashville heatmap.")
        return
    ...
    d = d[d[type_col] == tune_type].copy()
    if d.empty:
        print(f"[WARN] No chord rows for type={tune_type}; skipping chord_nashville heatmap.")
        return

    dom_spm = dominant_slots_per_measure(d, type_col, spm_col, tune_type)
    if dom_spm is None:
        print(f"[WARN] Could not determine dominant slots_per_measure for {tune_type}; skipping chord_nashville heatmap.")
        return
    ...
    # Top-N chord labels within this tune type
    top_labels = d[chord_col].astype(str).value_counts().head(top_n).index.tolist()
    d = d[d[chord_col].astype(str).isin(top_labels)].copy()
    ...
    ax.set_title(f"Top {top_n} chord_nashville Frequency by Slot ({tune_type.title()}, slots_per_measure≈{dom_spm})")
    ax.set_xlabel("slot_position")
    ax.set_ylabel("chord_nashville")
...
def main() -> None:
    ...
    plot_chord_nashville_heatmap_by_type(beat, outdir, tune_type="jig", outname="07_chord_nashville_heatmap_jigs.png", top_n=15)
    plot_chord_nashville_heatmap_by_type(beat, outdir, tune_type="reel", outname="08_chord_nashville_heatmap_reels.png", top_n=15)
    plot_chord_nashville_heatmap_by_type(beat, outdir, tune_type="jig", outname="07_chord_nashville_heatmap_jigs.png", top_n=15)
    plot_chord_nashville_heatmap_by_type(beat, outdir, tune_type="reel", outname="08_chord_nashville_heatmap_reels.png", top_n=15)
